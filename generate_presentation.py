...
ACCENT     = RGBColor(0xE9, 0x4F, 0x37)   # vivid red-orange
ACCENT2    = RGBColor(0x0F, 0x3D, 0x6E)   # mid navy (card bg)
...
def accent_bar(slide, top=0.72, height=0.06):
    """Horizontal accent line under the title."""
    box(slide, 0, top, 13.33, height, fill_color=ACCENT)
...
for (heading, body), (cl, ct) in zip(card_data, card_positions):
    box(s1, cl, ct, card_w, card_h, fill_color=ACCENT2,
        line_color=ACCENT, line_width_pt=1.5)
    txt(s1, heading, cl+0.18, ct+0.12, card_w-0.3, 0.5,
        font_size=15, bold=True, color=YELLOW)
    txt(s1, body, cl+0.18, ct+0.65, card_w-0.36, card_h-0.8,
        font_size=13, color=WHITE)

# Footer
box(s1, 0, 7.15, 13.33, 0.35, fill_color=ACCENT2)
...
txt(s2,
    ...
    0.5, 0.9, 12.3, 0.9, font_size=16, color=LIGHT_GREY)

txt(s2, "Both layers must be passed before we spend a single cent on an SMS.",
    0.5, 1.65, 12.3, 0.6, font_size=15, bold=True, color=YELLOW, align=PP_ALIGN.CENTER)
...
txt(s3, "🔒  AppRequestContextFilter  (Firebase App Check)",
    0.4, 0.85, 12.5, 0.55, font_size=18, bold=True, color=YELLOW)
...
for label, body, cl in flow:
    is_bot = "Bot" in label
    fill = RGBColor(0x8B, 0x00, 0x00) if is_bot else ACCENT2
    border = RGBColor(0xFF, 0x44, 0x44) if is_bot else ACCENT
    box(s3, cl, 1.6, 3.4, 2.9, fill_color=fill,
        line_color=border, line_width_pt=2)
    txt(s3, label, cl+0.15, 1.7, 3.1, 0.55,
        font_size=14, bold=True, color=YELLOW if is_bot else WHITE)
    txt(s3, body, cl+0.15, 2.3, 3.1, 2.0,
        font_size=12, color=WHITE)

# Arrows between boxes
for ax in [3.75, 7.50]:
    txt(s3, "➤", ax, 2.75, 0.5, 0.5, font_size=22, color=ACCENT, align=PP_ALIGN.CENTER)

# Outcome banner
box(s3, 0.35, 4.8, 12.6, 0.75, fill_color=ACCENT,
    line_color=ACCENT, line_width_pt=0)
txt(s3,
...
], 0.5, 5.75, 12.3, 1.3, font_size=14)

box(s3, 0, 7.15, 13.33, 0.35, fill_color=ACCENT2)
...
for i, (heading, body) in enumerate(whr):
    cl = 0.35 + i * 4.35
    box(s4, cl, 1.6, 4.1, 3.8, fill_color=ACCENT2,
        line_color=ACCENT, line_width_pt=1.5)
    box(s4, cl, 1.6, 4.1, 0.62, fill_color=ACCENT)
...
box(s4, 0.35, 5.65, 12.6, 1.05, fill_color=ACCENT2,
    line_color=YELLOW, line_width_pt=1.5)
txt(s4, "🔐  Combined Defence Summary",
    0.55, 5.68, 5, 0.45, font_size=14, bold=True, color=YELLOW)
bullet_block(s4, [
...
], 0.55, 6.1, 12.2, 0.6, font_size=12, icon="✔")
